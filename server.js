...
// REQUISITO 8 e 9: INDEXAÇÃO AVANÇADA DE GRAFO (Mapeando arquivos, [[Links]] e #Tags)
        ...
        files.forEach(file => {
            ...
            try {
                ...
                while ((match = linkRegex.exec(fullContent)) !== null) {
                    ...
                    // Encontra correspondência física do arquivo
                ...
                // Mapeamento de Hashtags #Tag (Assuntos e conexões semânticas)
...
// REQUISITO 8: CONVERSÃO AUTOMÁTICA DE QUALQUER EXTEÇÃO PARA MARKDOWN (.md) NO UPLOAD
app.post("/api/upload-vault", (req, res) => {
    try {
        let rawTarget = cleanPath(req.query.file);
        
        // Altera extensões .txt ou .docx para .md estruturado automaticamente
        ...
        // Se for um arquivo de texto avulso, envelopa semanticamente para o Obsidian
...
app.post("/api/chat", async (req, res) => {
    if (missingVars.length > 0) return res.status(500).json({ error: "Erro de Configuração." });
    ...
    try {
        ...
        let response = await anthropic.messages.create({
            ...
            max_tokens: 4000,
            system: "Você é o Assistente BMS, uma inteligência jurídica sênior operando em Modo Deus. Quando criar ou editar notas, use intensamente links [[NomeDoArquivo]] e hashtags tematizadas como #DireitoTrabalhista, #Prazo ou #Contradição para alimentar a malha neural do usuário.",
            messages,
            tools: localTools
        });
                ...
                if (block.type === "tool_use") {
                    ...
                    try {
                        if (block.name === "list_notes") toolResult = listNotesLocal();
                            ...
                            toolResult = { status: "Documento criado no Obsidian." };
                        }
                            ...
                            toolResult = { status: `Pronto: /downloads/${block.input.filename}` };
                        }
